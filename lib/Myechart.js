            ...
            setOption:function (option) {
               ...
               // 找到数据最大值
                ...
                for(var i=0;i<dataArr.length;i++){
                    ...
                    console.log( option.xAxis.data[i].length)
                    //文字应该向左偏移宽度
                ...
                legendText.forEach(function(data,index){
                        ...
                        ctx.moveTo(lengendBeginPathX,lengendBeginPathY);
                        ctx.lineTo(legendIconWidth,lengendBeginPathY);
                        ctx.lineTo(legendIconWidth,legendIconHight);
                        ctx.lineTo(lengendBeginPathX,legendIconHight);
                        ctx.lineTo(lengendBeginPathX,lengendBeginPathY);
                        //绘制最后一笔使图像闭合
                        ctx.lineJoin = "round";  
                        ctx.lineWidth = 10;
                        ctx.closePath();
